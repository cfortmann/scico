...
\begin{enumerate}[resume=b]
	...
	\begin{tabular}{l|l}
		...
		Explizit: Berechne $v[i,j]=\mathop{}\!\mathbin\bigtriangleup u=\dot{u}$ &Implizit: $u[i,j,t+1]=u[i,j,t]+\delta t \cdot \mathop{}\!\mathbin\bigtriangleup u[i,j,t+1]$ \\
		und dann $u[i,j,t+1]=u[i,j,t]+\delta t \cdot v[i,j]$ &  erfordert also das Lösen des GLs:$(I-\delta t )\mathop{}\!\mathbin\bigtriangleup u[t+1]=u[t]$\\
...
\begin{enumerate}[resume=b]
	...
	\fbox{\parbox[c]{\textwidth}{Der Eliminationsgraph ermittelt genau die Anzahl an Fill-ins die neu auftreten durch die \chf. Aufgestellt wird er so: Erstelle einen Konten für jede Spalte.
	Wenn in A  $a_{ij} \neq 0$ ist, dann ziehe eine Kante von vom Knoten i nach Knoten j. Dies ist der initial Graph, nun entferne suk­zes­si­v die Knoten(angefangen mit Knoten 1) bis nur noch 1 Knoten übrig ist.
	Wenn ein Weg zwischen zwei Knoten über den gelöschten Knoten führte, so ziehe eine neue Kante der diesen Weg wiederspiegelt. Alle so neu gezogenen Kanten stellen Fill-ins dar die in der \chf auftreten werden.}}
...
\begin{enumerate}[resume=b]
	...
	\fbox{\parbox[c]{\textwidth}{Zu beachten ist, dass manche Variablen private gesetzt werden müssenalso der Speicherzugriff eindeutig sein muss.  }}
