...
\begin{enumerate}[series=b]
	...
	\fbox{\parbox[c]{\textwidth}{Eine Basisfunktion beschreibt eine Kurve indem die Kurve als eine endliche Anzahl von Parametern beschreibt. Die von uns verwendete Basisfunktion ist die Monom-Basis $x^i$.}}
...
\begin{enumerate}[resume=b]
	...
	\item \textbf{ Wie stellt man das lineare Gleichungssystem für den Laplace auf?} \newline
	\fbox{\parbox[c]{\textwidth}{Jeder Punkt im Gitter ist eine Unbekannte die berechnet werden muss, wir haben als bei einem N x N mit Gitter N-2 Unbekannten (Randpunkte verändern sich nicht). Dies führt zu einem $(N-2)^2$ x $(N-2)^2$ Gleichungssystem $-Lu=b$.
